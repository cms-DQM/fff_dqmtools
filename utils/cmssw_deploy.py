...
def apply_multiple_pr(base_path, args, cmd_prefix=[]):
    ...
    for mr in status.keys():
        if status[mr][0] == 0:
            ...
            all_prs_applied = False
    return all_prs_applied
...
def make_release(sc, args):
    ...
    if args.pull_requests:
        ...
        success = apply_multiple_pr(base_path, args, cmd_prefix=cmd_prefix)

    if args.use_tmp:
        # Copy the CMSSW release from /tmp to the correct location
...
def parse_args():
    ...
    group.add_argument(
        "--use-tmp",
        action="store_true",
        help="Do scram stuff in tmp (work around against CMSONS-8215)",
    )
